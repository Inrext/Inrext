...
const mainHandler = async (req, res) => {
  ...
  try {
    ...
    if (req.method === "PATCH") {
      ...
      const { id } = req.query;
      if (!id) {
        return res.status(400).json({ success: false, message: "Property id is required for update" });
      }
    ...
    if (req.method === "DELETE") {
      ...
      const { id } = req.query;
      if (!id) {
        return res.status(400).json({ success: false, message: "Property id is required for delete" });
      }
