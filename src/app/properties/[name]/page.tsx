    ...
    const fetchProperty = async () => {
      try {
        ...
        if (data && data._id) {
          ...
          // Only show children whose parentId matches this property _id (handle ObjectId vs string)
          const parentIdStr = typeof data._id === 'object' && data._id !== null && ('$oid' in data._id)
            ? data._id.$oid
            : String(data._id);
          const children = (subResult.data || []).filter((child) => {
            if (!child.parentId) return false;
            const childParentId = typeof child.parentId === 'object' && child.parentId !== null && ('$oid' in child.parentId)
              ? child.parentId.$oid
        ...
        <div className="grid lg:grid-cols-5 grid-cols-1 lg:gap-x-5 gap-y-5">
          <div className="col-span-2 relative flex flex-col justify-center lg:gap-y-[1.5rem] gap-y-[0.8rem] lg:mx-10">
            ...
            >
              ...
              {property.minPrice}{property.sizeUnit ? ` / ${property.sizeUnit}` : ''}
            </p>
            {/* <p
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                {subProperties.map((child, idx) => {
                  ...
                  return (
                    <div
                      key={child._id || idx}
                      className={`rounded-xl shadow-lg border flex flex-row transition-colors duration-300 overflow-hidden w-full max-w-3xl ${isDarkMode ? 'bg-black border-white text-white' : 'bg-white border-blue-200 text-black'}`}
                      ...
                      <div className="flex flex-col justify-between p-2">
                        <div>
                          ...
                          <div className="flex flex-row gap-4 mt-2">
                            {/* <span className="text-sm"><b>Type:</b> {child.propertyType || '-'}</span> */}
                          ...
                          {/* Features Row (optional, keep if needed) */}
